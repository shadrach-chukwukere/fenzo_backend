...
app.post("/api/checkout", async (req, res) => {
  const { user_id, items, total_amount, payment_method, delivery_fee } =
    req.body;
...
app.post("/api/signup", async (req, res) => {
  ...
  try {
    ...
    const [existing1] = await db.query("SELECT * FROM users WHERE phone = ? ", [
      phone,
    ]);
  ...
  try {
    ...
    if (!query) {
      ...
      finalValues = [
        ...
        likeQuery, // price
        likeQuery,
        likeQuery, // exclusions for title + description
...
app.get("/api/totalOrder", async (req, res) => {
  try {
    const [results] = await db.query(
      "SELECT order_date, total_orders FROM orderrate ORDER BY order_date ASC"
    );
    res.json(results);
  } catch (err) {
    console.error("❌ MySQL error:", err);
...
app.delete("/api/product/delete/:id", (req, res) => {
  ...
  db.query(deleteOrderItems, [productId], (err1, result1) => {
    if (err1) {
      ...
      return res
        .status(500)
        .json({ message: "Failed to delete related order items" });
    }
  ...
  try {
    ...
    if (!category || !products || !Array.isArray(products)) {
      return res
        .status(400)
        .json({
          message: "Invalid input. Category and product array are required.",
        });
    }
