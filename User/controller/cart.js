...
// 🔹 Helper function to fetch a user's cart
...
export async function addToCart(req, res) {
  ...
  try {
    ...
    const [stockRow] = await db.query("SELECT stock FROM products WHERE id = ?", [
      productId,
    ]);
...
export async function updateCart(req, res) {
  ...
  try {
    const [productRows] = await db.query(`SELECT stock FROM products WHERE id = ?`, [
      productId,
    ]);
...
export async function hasInCart(req, res) {
  ...
  try {
    ...
    console.error("Has in cart error:", err);
